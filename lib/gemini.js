 ...
 */
export async function rewriteContent({ rawText, imageInfos, imageData = [], tone, category }) {
    ...
    // Attach actual images for Gemini Vision analysis
    if (imageData && imageData.length > 0) {
        for (let i = 0; i < imageData.length; i++) {
            const img = imageData[i];
            if (img.base64 && img.mimeType) {
                userParts.push({
                    text: `\n[이미지 ${i + 1} 시작 — AI가 이 이미지를 분석하여 본문에 자연스럽게 설명을 녹여주세요]`
                });
                userParts.push({
