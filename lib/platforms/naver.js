...
export async function publishToNaver(post, cookies, blogId = null) {
    try {
        // blogId 결정 — 세션에서 먼저 확인
        let activeBlogId = blogId;

        // ===== 1차: Chrome 프로필 기반 자동 발행 =====
        try {
            const { publishToNaverPuppeteer, getSessionStatus } = await import('./naver-puppeteer.js');
            const session = getSessionStatus();

            if (session.profileExists) {
                console.log('[Naver] Chrome 프로필 모드로 발행 시도...');
