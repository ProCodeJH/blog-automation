...
export async function validateNaverCookies(cookies) {
    try {
        ...
        const res = await fetch('https://blog.naver.com/MyBlogMain.naver', {
            method: 'GET',
            headers: {
                'Cookie': parsed,
        ...
        // 블로그 아이디 추출
        const blogIdMatch = html.match(/var\s+blogId\s*=\s*["']([^"']+)["']/) ||
            html.match(/blogId=([a-zA-Z0-9_]+)/) ||
            html.match(/"blogId":"([^"]+)"/);

        if (blogIdMatch) {
            return { valid: true, blogId: blogIdMatch[1], message: `✅ 인증 성공 (${blogIdMatch[1]})` };
        }

        // blogId를 못 찾아도 200이면 일단 성공으로 처리
        if (res.status === 200) {
            return { valid: true, blogId: null, message: '✅ 인증 성공 (블로그 ID 확인 필요)' };
        }

        return { valid: false, error: '인증 실패. 쿠키를 다시 확인해주세요.' };
    } catch (error) {
