...
async function launchBrowser({ headless = true, useProfile = false } = {}) {
    ...
    if (useProfile) {
        ...
        // ★ 이전 브라우저가 비정상 종료 시 잠금 파일 제거
        const lockFile = path.join(PROFILE_DIR, 'SingletonLock');
        if (fs.existsSync(lockFile)) {
            try { fs.unlinkSync(lockFile); console.log('[Tistory] SingletonLock 제거됨'); } catch { }
        }
...
export async function publishToTistoryPuppeteer({
    ...
    try {
        // STEP 1: 쿠키 주입 (프로필 모드에서도 폴백으로 쿠키 주입)
        ...
        try {
            await page.waitForSelector('iframe, .mce-content-body, textarea[placeholder*="제목"], #post-title-field', { timeout: 20000 });
        ...
        // 로그인 체크 — 리다이렉트됐으면 인라인 로그인 시도
        let currentUrl = page.url();
        if (currentUrl.includes('/auth/login') || currentUrl.includes('accounts.kakao.com')) {
            console.log('[Tistory] 로그인 필요 — 자동 로그인 대기 중... (최대 30초)');
            // 카카오 자동 로그인 대기 (Chrome 프로필에 저장된 계정 사용)
            for (let i = 0; i < 30; i++) {
                await new Promise(r => setTimeout(r, 1000));
                currentUrl = page.url();
                if (!currentUrl.includes('/auth/login') && !currentUrl.includes('accounts.kakao.com')) {
                    console.log('[Tistory] ✅ 자동 로그인 성공!');
                    break;
                }
                if (i === 29) {
                    console.log('[Tistory] ⏰ 자동 로그인 타임아웃');
                    await browser.close();
                    return {
                        success: false,
                        error: '티스토리 로그인이 필요합니다. tistory-login.mjs를 실행하세요.',
                        needLogin: true,
                    };
                }
            }
            // 로그인 후 에디터 재접속
            try {
                await page.goto(`https://${blogId}.tistory.com/manage/newpost`, {
        ...
        // STEP 8: 발행 결과 확인 (URL 변경 polling — networkidle2는 3분+ 걸림)
        console.log('[Tistory] 발행 결과 대기 (최대 30초)...');
        let finalUrl = page.url();
        for (let i = 0; i < 30; i++) {
            await new Promise(r => setTimeout(r, 1000));
            finalUrl = page.url();
            if (!finalUrl.includes('/manage/newpost')) {
                console.log(`[Tistory] URL 변경 감지 (${i + 1}초): ${finalUrl}`);
                break;
            }
        }
        ...
        // 발행 성공 판단 (글 관리 페이지 리다이렉트도 성공)
        const isPublished = !finalUrl.includes('/manage/newpost') &&
            (finalUrl.includes('.tistory.com/') &&
                (/\/\d+$/.test(finalUrl) || finalUrl.includes('/manage/') || finalUrl.includes('/entry/')));
        ...
        try {
            await page.goto(`https://${blogId}.tistory.com/manage/posts`, { waitUntil: 'domcontentloaded', timeout: 15000 });
