 ...
 */
export function buildEditPrompt({ rawText, imageInfos, imageData = [], tone = 'friendly', category = '' }) {
  const toneDesc = TONE_DESCRIPTIONS[tone] || TONE_DESCRIPTIONS.friendly;

  const hasVisualData = imageData && imageData.some(d => d.base64);

  const imageDescriptions = imageInfos
    .map((img, i) => `  이미지 ${i + 1}: ${img.memo || '(메모 없음)'}`)
    .join('\\n');

  // Scale recommended text length based on image count
  const imgCount = imageInfos.length || 0;
  const recommendedMin = Math.max(1500, imgCount * 350);
  const recommendedMax = Math.max(3000, imgCount * 500);

  const visualAnalysisBlock = hasVisualData ? `

【🔍 이미지 시각 분석 지시사항 (중요!)】
이 요청에는 실제 이미지가 첨부되어 있습니다. 각 이미지를 꼼꼼히 분석하고:
1. 이미지에 보이는 장면, 사물, 분위기, 색감을 파악하세요
2. 해당 이미지가 배치될 본문 전에 이미지 내용과 자연스럽게 연결되는 서술을 작성하세요
3. 이미지 아래 캡션은 이미지에서 실제로 보이는 내용을 반영하세요
4. 이미지의 감정/분위기를 글의 해당 섹션 톤에 녹여넣으세요
5. "이 사진에서 볼 수 있듯이", "위 이미지처럼" 같은 직접 참조도 자연스럽게 사용하세요
` : '';
...
【첨부 이미지 정보 (총 ${imgCount}장, 순서대로)】
${imageDescriptions}
${visualAnalysisBlock}
【글 분량 가이드】
이미지 ${imgCount}장 기준, 본문 ${recommendedMin}~${recommendedMax}자 권장 (이미지 1장당 300~500자)

【사용자의 러프 초안】
${rawText}

위 초안을 파워블로거 스타일로 완성해주세요. 이미지 ${imgCount}장을 적절한 위치에 배치하고, ${hasVisualData ? '각 이미지의 실제 내용을 분석하여 자연스럽게 글에 녹여주세요.' : '이미지 메모를 참고하여 배치해주세요.'}
